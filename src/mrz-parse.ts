...
export const mrzParseTwoLine = (line1: string, line2: string) => {
  const parseResultInfo: any = ref(['']);
  let passportMRZ = reactive({} as any);
...
export const mrzParseThreeLine = (line1: string, line2: string, line3: string) => {
  const parseResultInfo: any = ref(['']);
  let passportMRZ = reactive({} as any);
  let type = line1.substring(0, 1);
  if(!(/[I|P|V]/.test(type))) return false;
