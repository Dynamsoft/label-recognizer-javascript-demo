...
export default defineComponent({
    setup() {
      ...
      async function video_OnWindowResize() {
        if(document.body.clientWidth >= 980) {
          setRegion(25,40,75,40,75,60,25,60);
        } else {
          setRegion(5,40,95,40,95,60,5,60);
        }
      }

      onMounted(async() => {
        let cameraEnhancer: DCE;
        DLR.onResourcesLoadStarted = () => { isShowMask.value = true; }
        DLR.onResourcesLoaded = () => { isShowMask.value = false; }
        (DLR as any).onResourcesLoadProgress = (_: string,progress: {loaded: number, total: number}) => { progressRate.value = (progress.loaded / progress.total) * 100; }
        const internalInstance: any = getCurrentInstance();
        cameraEnhancer = await DCE.createInstance();
        await cameraEnhancer.setUIElement(internalInstance.refs.elRefs);
        recognizer.value = await DLR.createInstance({runtimeSettings: runtimeMode.value});
        recognizer.value.cameraEnhancer = cameraEnhancer;
        (recognizer.value as any).whenToPlaySoundforSuccessfulRead = true;
        document.body.clientWidth >= 980 ? await setRegion(25,40,75,40,75,60,25,60) : await setRegion(5,40,95,40,95,60,5,60);
        window.addEventListener('resize', video_OnWindowResize);

        recognizer.value.onFrameRead = async (results: any) => {
          if(results.length !== 0 && (runtimeMode.value === 'video-number' || runtimeMode.value === 'video-letter' || runtimeMode.value === 'video-vin')) {
            let resultArr: Array<string> = [];
            results[0].lineResults.forEach((res: any) => {
              if(res.text.startsWith("V1N")) {
                res.text = res.text.substring(3);
              }
        ...
        recognizer.value.onUniqueRead = async (txt: string, _: any) => {
          /* let currentFrame = recognizer.value.cameraEnhancer.getFrame({
            regionBottom: 60,
            regionRight: 75,
            regionLeft: 25,
            regionTop: 40,
          ...
          recognizerFrame.value = currentFrame.canvas;
      
          isShowScanningPrompt.value = false;
          if(runtimeMode.value === 'video-vin') {
            if(txt.startsWith("V1N")) {
              txt = txt.substring(3);
            }
            recognizeResultInfo.value = txt;
            isShowResults.value = true;
            await recognizer.value.pauseScanning();
          }
          clearTimeout(timer); */
        }
                  
        recognizer.value.onMRZRead = async (_: string, recognizeResults: any) => {
          recognizer.value.ifSaveOriginalImageInACanvas = true;
          let region = document.body.clientWidth >= 980 ? webRegionArea : mobileRegionArea;
          let currentFrame = recognizer.value.cameraEnhancer.getFrame(region);
          recognizerFrame.value = currentFrame.canvas;
          recognizer.value.ifSaveOriginalImageInACanvas = false;

          let resultArr: Array<string> = [];
          recognizeResults.forEach((res: any) => {
            resultArr.push(res.text);
          })
          if(resultArr.length >= 2) {
            if(resultArr[0].length === 30 && resultArr.length === 3) {
              let parseResultInfo = mrzParseThreeLine(resultArr[0], resultArr[1], resultArr[2]);
              if(parseResultInfo) {
                await showResults(recognizer.value, parseResultInfo, resultArr);
              } else {
                recognizer.value.pauseScanning();
                recognizer.value.resumeScanning();
              }
            } else if(resultArr[0].length === 36 || resultArr[0].length === 44) {
        ...
        let cl: any = await recognizer.value.cameraEnhancer.getAllCameras();
        cameraList.value = cl;
        scanningText.value = true;
        if(document.body.clientWidth < 980 && runtimeMode.value === 'video-mrz') {
          proxy.$message.open({
      ...
      const showResults = async (r: any, parseResult: object, resultArr: Array<string>) => {
        recognizeResultInfo.value = resultArr;
        clearTimeout(timer);
        isShowScanningPrompt.value = false;
        parseResultInfo.value = parseResult;
        isShowResults.value = true;
        r.pauseScanning(true);
      }

      return () => (
        <>
          <div class="currentMode">{runtimeMode.value === 'video-mrz' ? 'MRZ Scanner' : runtimeMode.value.substring(6).toUpperCase()}</div>
          <div class="scanningText" v-show={scanningText.value}>{`Align the frame within the ${formatModeName(runtimeMode.value)} and start scanning`}</div>
          <div class="scanningPrompt" v-show={isShowScanningPrompt.value && scanningText.value}>
            <div v-show={runtimeMode.value === 'video-mrz'}>
              <div>P&lt;CANAMAN&lt;&lt;RITA&lt;TANIA&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;</div>
              <div>ERE82721&lt;9CAN8412072M2405251&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;&lt;04</div>
            </div>
            <div v-show={runtimeMode.value === 'video-vin'}>
              <div>2C4RDGCG0FR805928</div>
            </div>
            <div v-show={runtimeMode.value === 'video-letter'}>
              <div>ABCDEFGHIJK....</div>
            </div>
            <div v-show={runtimeMode.value === 'video-number'}>
              <div>123456789...</div>
            </div>
          </div>
          <div ref="elRefs" id="div-video-container" style="width:100%;height:100%;min-width:100px;min-height:100px;background:#eee;position:relative;" onClick={ closeAllList }>
            ...
            <div class="dlr-msg-poweredby" style="position:absolute;left:50%;bottom:10%;transform:translateX(-50%);">
...
@keyframes dlr-scanlight{from{top:0;}to{top:97%;}}
.currentMode {
  position: absolute;
  top: 8%;
  left: 50%;
